...
// 静态成员初始化,7-bit 字母表.
string CBase64::m_sBase64Alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
//---------------------------------------------------------------------------
string CBase64::Encode(LPSTR szEncoding,int nSize)
{
	string sOutput = "";
	int nNumBits;
	UINT nDigit;
...
int CBase64::Decode(LPSTR szDecoding, LPSTR szOutput)
{
    int c, lp =0;
	int nDigit;
    int nDecode[ 256 ];
...
//---------------------------------------------------------------------------
void CBase64::WriteBits(UINT nBits, LPSTR szOutput, int& i)
{
	UINT nScratch;
